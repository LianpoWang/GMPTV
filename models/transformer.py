...
class GaussianAttention(nn.Module):
    def __init__(self, d_model, k, sigma=0.2, sc=0.1):
    ...
    def forward(self, q, k, v, xyz, knn_xyz):
        B, N, C = q.shape
        attn = (q[:, :, None, :] - k).pow(2).sum(dim=-1)
        attn = self._func_gauss(attn, xyz, knn_xyz) * self.scaling
        attn = attn.softmax(dim=-1)
        return (attn.unsqueeze(-1) * v).sum(dim=2)

    def _func_gauss(self, attn, xyz, knn_xyz):
        B, N, K, C = knn_xyz.shape
        device = attn.device
        k = self.k
        ...
        ker_S = gauss_kernel + self.lr_m / self.sc
        attn_gauss = ker_S.view(1, 1, 1, self.k ** 2).repeat(B, N, 1, 1) * attn
        return attn_gauss


class GCLBlock(nn.Module):
    def __init__(self, d_points, d_model, k, sigma=0.2) -> None:
        super().__init__()
        self.fc1 = nn.Linear(d_points, d_model)
        self.fc2 = nn.Linear(d_model, d_points)
        self.fc_delta = nn.Sequential(
        ...
        self.w_vs = nn.Linear(d_model, d_model, bias=False)
        self.k = k
        self.gaussian_attn = GaussianAttention(d_model, k, sigma)
    ...
    def forward(self, xyz, features):
        ...
        attn = self.gaussian_attn(q, k, v + pos_enc, xyz, knn_xyz)
        res = self.fc2(attn) + pre
        return res


class GCL(nn.Module):
    def __init__(self, num_layers=1, d_points=16, ffn_dim_expansion=4, sigma=0.2):
        super(GCL, self).__init__()
        self.d_points = d_points
        self.d_model = d_points * ffn_dim_expansion
        self.layers = nn.ModuleList([
            GCLBlock(d_points=self.d_points, d_model=self.d_model, k=16, sigma=sigma)
            for i in range(num_layers)
        ])
        for p in self.parameters():
            if p.dim() > 1:
    ...
    def forward(self, xyz0, xyz1, feature0, feature1):
        ...
        feature0 = feature0.permute(0, 2, 1).contiguous()
