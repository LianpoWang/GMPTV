...
from loss.fluidflow_loss import sceneflow_loss_func
from evaluate.evaluate import validate_things, validate_fluidflow3d, validate_waymo
from models.gmptv import GMPTV
...
def get_args_parser():
    ...
    parser.add_argument('--stage', default='fluidflow3d', type=str,
                        help='training stage on different datasets (fluidflow3d / things_subset / things_subset_non-occluded / things_flownet3d / waymo )')
    parser.add_argument('--val_dataset', default=['fluidflow3d'], type=str, nargs='+',
                        help='waymo / things / fluidflow3d')
    # training
    ...
    # resume pretrained model or resume training  default=None
    parser.add_argument('--resume', default='checkpoints/GMPTV_step_100000.pth', type=str,
...
def main(args):
    ...
    model = GMPTV(backbone=args.backbone,
    ...
    while total_steps < args.num_steps:
        ...
        for i, sample in enumerate(train_loader):
            ...
            fluidflow_preds = results_dict_point['flow_preds']

            loss, metrics_3d = fluidflow_loss_func(fluidflow_preds, flow_3d)
            ...
            #     checkpoint_path = os.path.join(args.checkpoint_dir, 'GMPTV_step_%06d.pth' % total_steps)
            #     torch.save({
            ...
            #     checkpoint_path = os.path.join(args.checkpoint_dir, 'GMPTV_checkpoint_latest.pth')
            #     torch.save({
            #         'model': model_without_ddp.state_dict(),
            #         'optimizer': optimizer.state_dict(),
            #         'step': total_steps,
...
def val(args):
    ...
    if args.eval:
        ...
        if 'fluidflow3d' in args.val_dataset:
            start = timeit.default_timer()
            results_dict = validate_fluidflow3d(args.stage,
                                          model_without_ddp,
                                          )
            val_results.update(results_dict)
            stop = timeit.default_timer()
